  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        <p className="text-[13px] text-foreground font-medium leading-relaxed mb-1.5 truncate overflow-hidden">
          {fb.message}
        </p>
        <div className="flex gap-3 items-center flex-wrap">
          <span className="text-[11px] text-[#3d3d3d] font-mono">{fb.pageUrl}</span>
          {fb.email && (
            <span className="text-[11px] text-muted-foreground">{fb.email}</span>
          )}
          <span className="text-[11px] text-[#3d3d3d]">{fb.createdAt}</span>
        </div>
      </div>
