...
const feedbackRows = [
  { msg: "Checkout fails on mobile Safari. Cart empties after payment.", tag: "new", tagColor: "text-primary bg-primary/10" },
  { msg: "Love the new dashboard! Much faster than before.", tag: "reviewed", tagColor: "text-info bg-info/10" },
  { msg: "Can you add CSV export for the reports section?", tag: "new", tagColor: "text-primary bg-primary/10" },
  { msg: "The filter dropdown doesn't close when clicking outside.", tag: "resolved", tagColor: "text-success bg-success/10" },
];
          ...
          <div className="bg-card border border-border/70 rounded-2xl overflow-hidden shadow-card-foreground">
            ...
            <div className="h-11 border-b border-border flex items-center px-4 gap-2 bg-card">
              ...
              <div className="flex-1 h-6 bg-background rounded ml-2 flex items-center pl-2.5">
                <span className="text-[11px] text-muted-foreground/40 font-mono">app.feedlyte.com/dashboard</span>
              </div>
            </div>
            ...
            <div className="flex h-95">
              ...
              <div className="flex-1 p-6 overflow-hidden">
                <div className="mb-5">
                  <div className="text-base font-bold text-foreground mb-1">All Feedback</div>
                  <div className="text-xs text-muted-foreground/50">24 entries across 3 projects</div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2.5 mb-5">
                  {stats.map((stat) => (
                    <div key={stat.label} className="bg-background border border-border rounded-lg p-3">
                      <div className="text-[10px] text-muted-foreground/40 uppercase tracking-widest mb-1">{stat.label}</div>
                ...
                <div className="flex flex-col gap-2">
                  {feedbackRows.map((item, i) => (
                    <div key={i} className="bg-background border border-border rounded-lg px-3.5 py-2.5 flex items-center justify-between gap-3">
                      <span className="text-xs text-muted-foreground flex-1 truncate">{item.msg}</span>
                      <span className={`text-[10px] font-semibold uppercase tracking-widest px-2 py-0.5 rounded-full shrink-0 ${item.tagColor}`}>
                        {item.tag}
                      </span>
                    </div>
                  ))}
